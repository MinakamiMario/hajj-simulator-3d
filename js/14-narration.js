...
// ============================================================
// De gids spreekt geen Arabisch meer uit (Nederlandse stem → niet verstaanbaar).
// Het Arabisch blijft op het scherm staan (NARRATION) en wordt op de juiste
// momenten ingesproken door de klas-opnames (zie Recite.AT in 16-recite.js):
//   2_2 → Talbiya,  4_2 → Rabbana atina.
const NARRATION_SAY = {
  '0_1': "Voel je hart maar — daar, in alle rust, maak je je intentie. De Niyyah leeft in je hart; daar komt het op aan. Bij de Miqaat spreken velen er straks ook woorden bij — maar de kern is je stille besluit: o Allah, voor U onderneem ik deze reis.",
  '2_2': "En dan, uit duizenden monden tegelijk, klinkt de Talbiya. Luister even... en zeg het zachtjes mee.",
  '4_2': "Tussen de twee hoeken — de Jemenitische hoek en de Zwarte Steen — spreek je een bekende smeekbede. Ze betekent: Heer, geef ons het goede in dit leven én in het hiernamaals, en bescherm ons tegen het Vuur.",
  '6_0': "Dit is het hart van de Hajj. De Profeet zei het kort: de Hajj ís Arafat. Wie deze dag hier staat, heeft de Hajj.",
  '11_1': "Naast de moskee ligt al-Baqi, waar veel metgezellen rusten. Groet hen met vrede en wens hun: vrede zij met jullie, bewoners van deze plaats."
};
...
const GidsAudio = {
  ...
  play(scene,idx){
    ...
    const a=new Audio(this.base+'scene'+scene+'_'+idx+this.ext+'?v='+BUILD);
    this.cur=a;
    a.addEventListener('playing',()=>{ if(window.Gids)clearTimeout(Gids.timer); });   // audio stuurt het tempo
    a.addEventListener('ended',()=>{ if(this.cur!==a)return;
      // na de Nederlandse regel: spreekt de klas hier het Arabisch in? speel het, ga dan pas verder
      if(window.Recite && Recite.afterNarration && Recite.afterNarration(scene,idx,()=>{ if(window.Gids)Gids.next(); })) return;
      if(window.Gids)Gids.next(); });
